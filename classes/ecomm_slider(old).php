...
if( !class_exists('e_comm_slider') ){	
	class e_comm_slider {	  
			...
			return preg_replace('/title="[^"]*"/', 'title="'.$title.'"', get_the_post_thumbnail($my_post->ID, $thumb_size) );
               ...
               function get_slider_sidebar_contents($title,  $my_post= NULL){
                    ...
                    // $image_url = wp_get_attachment_url(get_post_thumbnail_id($my_post->ID));
                     ...
                     $return_arr = compact('bar_title','bar_slug','image_url','bar_content','bar_excerpt','bar_link_arr');
                     return $return_arr;
               }
		...
		function option_management(){
                    ...
                    //  if ( wp_verify_nonce($_REQUEST['nonce'], ECOMM_VALIDATE_KEY) ) {
                          ...
                          if(empty($cat_slug)){
                           ...
                           $comm_posts = $this->posts;
                            ...
                            $slider_main_body ='<div class="panel panel-1" >
                            ...
                            if(is_array($comm_posts) and count($comm_posts)>0) {
                                $slider_main_body = $this->get_show_slider_main_body_contents($this->posts,$show_sidebar);
                           ...
                           $content_return = array('return_post'=>count($this->posts),'status' =>'success','message'=> array('options'=>$options,'header_nav'=>$header_nav, 'main_body' =>$slider_main_body,'thumbnail_nav'=>$thumbnail_nav, 'thumbnail_content'=>$thumbnail_content,'sidebar'=>$sidebar_part,'numberofposts'=>$this->options['numberposts'], 'total_posts'=>$this->slide_total_count)); 
                            //for ajax process
			...
			$query = array('numberposts' => $number_of_page,"nopaging"=>true,'post_type' => $post_type);
			...
			if( !has_filter('stjoseph_ecomm_slider_custom_query_results') ) {
                ...
                function get_show_slider_main_body_contents( $ecomm_posts, $show_sidebar=0 ){
                    $main_body_contents = '';
                    if(!$ecomm_posts ){ 
                        return "";
                    }
                    ...
                    foreach($ecomm_posts as $post):
                            ...
                            $thumb_collection[]   = $this->get_slider_thumb($title, $post);
                            ...
                            $values = compact('id', 'title', 'excerpt', 'image', 'destination_url', 'target', 'display_title', 'display_excerpt'); 
                            extract($values);
                            ...
                            if( !$show_sidebar ){ //sidebar not showing, put here,need to be done 
                ...
                function get_ecomm_slider_sidebar_contents(){
                    $sidebar_contents = ''; 
                    $collections =  $this->sidebar_collection;
                    $share_link  = ECOMM_HOME_URL.'/'.$this->post_type.'/'.$this->parent_postslug.'/';
                    ...
                    $sidebar_contents .='<div id="e_comm_slider_sidebar_wrapper">';
                         foreach($collections as $key => $sidebar):
                               ...
                               //$sidebar_contents .='<span class="caption">'.(!empty($sidebar['bar_excerpt']))?$sidebar['bar_excerpt']:''.'</span>';
                                ...
                                $sidebar_contents .='<a href="http://twitter.com/share" class="twitter-share-button" data-count="horizontal" data-via="wherecanada">Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script>';
                                $sidebar_contents .='</br>';
                                $sidebar_contents .='<a href="http://pinterest.com/pin/create/button/?url='.$link_hash.'&media=" class="pin-it-button" count-layout="horizontal"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" /></a>';
                ...
                function get_show_slider_thumbnail_nav_contents(){
                    ...
                    if(is_array($thumb_collection) and count($thumb_collection) > 0) {
                        $values =  array('id' => $id, 'title' => $title, 'thumbs' => $thumb_collection, 'width' => $width);
                        extract($values);
                        $page_count = 1;
                        ...
                        $current_index = $key + 1;
                        $thumbnail_nav_contents .= '<li class="'.$current_index.'">';
                        $thumbnail_nav_contents .= $thumb; 
			...
			if( $promo_posts ){
				...
				ob_start(); ?>
                                <div id="slider_contents">
                                    <div id="slider_body"> 
                                        <?php do_action('before_stjoseph_ecomm_slider', $id); ?>
                                        <div<?php echo $wrapper_atts; ?> >

                                                <div<?php echo $slider_atts; ?>>

                                                        <?php foreach($promo_posts as $post): setup_postdata($post);
                                                                // Setup values to be passed to the stjoseph_ecomm_slider_content action hook //
                                                                ...
                                                                $sidebar_collection[] = $this->get_slider_sidebar_contents($title);
                                                                // Fetch link settings
                                                                ...
                                                                <div class="panel panel-<?php echo $i; ?>">
                                                                        ...
                                                                        <?php // Use the stjoseph_ecomm_slider_content action to populate the contents for panel
                                                                        do_action('stjoseph_ecomm_slider_content', $values); ?>
                                                                </div>
                                                ...
                                                if( $i >= 2 ) {
                                                    do_action('stjoseph_ecomm_slider_thumbnail_nav', array('id' => $id, 'thumbs' => $thumb_collection) );
                                                }
                                                ?>

                                        </div><?php
                                         do_action('after_stjoseph_ecomm_slider', $id);
                                        ?>
                                    </div> <!-- End for slide body -->
                                <div id="slider_sidebar">
                                  <?php   
                                   if( $i >= 2 ){
                                      $this->show_slide_post_number_area(); 
                                      do_action('stjoseph_ecomm_slider_sidebar', array('id' => $id, 'sidebars' => $sidebar_collection));
                                   }
                                   ?>  
                                </div>    
